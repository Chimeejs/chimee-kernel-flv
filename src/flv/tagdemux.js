...
export default class tagDemux {
    ...
    _parseAVCVideoData(arrayBuffer, dataOffset, dataSize, tagTimestamp, tagPosition, frameType, cts) {
        ...
        let drop = false;
        // 清理aac对齐数据
        for(let i = 0; i < this._timeInfo.aacFixedRecord.length; i++) {
        ...
        if(this._timeInfo.avcLastDts) {
            ...
            if(avcDts - this._timeInfo.avcLastDts > this._timeInfo.avcSampleDuration) {
                let fixByAudio = false;
                // 尝试利用acc的时间修正记录this._timeInfo.aacFixedRecord为参照修正视频时间，进行对齐
                for(let i = 0; i < this._timeInfo.aacFixedRecord.length; i++) {
                    if(avcDts >= this._timeInfo.aacFixedRecord[i].st && avcDts <= this._timeInfo.aacFixedRecord[i].ed) {
                        avcDts = avcDts -this._timeInfo.aacFixedRecord[i].fix;
                        fixByAudio = true;
                        // 修正后是否仍然有时间跳变
                        if(this._timeInfo.aacFixedRecord[i].ed == Number.POSITIVE_INFINITY && 
    ...
    _parseAudioData(arrayBuffer, dataOffset, dataSize, tagTimestamp) {
        ...
        if (aacData.packetType === 0) { // AAC sequence header (AudioSpecificConfig)
            ...
            const dtsDiff = dts - Math.floor(this._timeInfo.aacFirstDts + (this._timeInfo.aacCurrentDtsOffset)) - this._timeInfo.aacCurrentFixedLen;
            if(dtsDiff > 1) {
            ...
            // aac帧长度
            const aacFrameLen = 1024 * 1000 / meta.audioSampleRate;
            // 通过aac帧长度累加计算dts
            const aacDts = Math.floor(this._timeInfo.aacFirstDts + this._timeInfo.aacCurrentDtsOffset);
            const aacSample = { 
                unit: aacData.data,
                dts: aacDts,
                pts: aacDts,
